...
# Đồng bộ cameras từ MongoDB khi khởi động
...
async def log_requests(request: Request, call_next):
    ...
    # Lấy body nếu có (chỉ cho POST/PUT)
    try:
...
async def notify_signaling_server(camera):
    try:
        async with httpx.AsyncClient() as client:
            ...
            logger.info(f"Notified signaling server about camera {camera['id']}")
...
async def add_camera(camera: dict):
    try:
        ...
        await notify_signaling_server(camera)

        return {"message": "Camera saved", "camera": camera}
    except Exception as e:
        logger.error(f"Error saving camera: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error saving camera: {str(e)}")
