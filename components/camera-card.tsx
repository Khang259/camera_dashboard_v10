...
import { RefreshCw, Camera as CameraIcon, Video, VideoOff, Maximize, X as CloseIcon, Minimize } from "lucide-react";
...
export const CameraCard = React.memo(function CameraCard({ camera, ws, registerCallback }: CameraCardProps) {
  ...
  const captureScreenshot = () => {
    if (videoRef.current) {
      ...
      if (ctx) {
        ...
        link.href = dataUrl;
        link.download = `camera_${camera.id}_screenshot_${new Date().toISOString().replace(/[:.]/g, '-')}.png`;
        link.click();
  ...
  return (
    <div
      ...
      style={isZoomed ? { width: '150%', height: '150%', maxWidth: 'none', maxHeight: 'none' } : {}}
    >
      <video
        ref={videoRef}
        autoPlay
        playsInline
        muted
        className={`w-full h-full object-cover transition-transform duration-300 ${isConnected ? "block" : "hidden"}`}
        onError={(e) => {
      ...
      {isConnected && (
        <div className="absolute bottom-2 right-2 flex gap-2">
          ...
          <button
            onClick={expandVideo}
            className="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-lg transition-colors"
            title={isZoomed ? "Minimize video" : "Maximize video"}
          >
